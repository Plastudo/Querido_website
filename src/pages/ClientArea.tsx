...
import { getSavedBudgets, deleteBudget, type SavedBudget } from '../utils/storage';
...
function BudgetCard({ budget, onOpen, onDelete }: CardProps) {
  ...
  const { project, title, savedAt, grandTotal } = budget;
  const divisionCount = project.divisions.length;
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '20px 24px', flex: 1, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {/* Metadados: região + nº de divisões */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 13, color: 'var(--color-text-secondary)' }}>
            ...
            {REGION_LABEL[project.region] ?? project.region}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 13, color: 'var(--color-text-secondary)' }}>
            <svg width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--color-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 2 }}>
            Total estimado
          </div>
          <div style={{ fontSize: 20, fontWeight: 600, color: 'var(--color-primary)', letterSpacing: -0.4 }}>
            {fmt(grandTotal)}
          </div>
        </div>
            ...
            <>
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 'var(--radius-button)', fontSize: 13, fontWeight: 500,
...
export default function ClientArea() {
  ...
  const [budgets, setBudgets] = useState<SavedBudget[]>(() => getSavedBudgets());
  ...
  const handleOpen = (budget: SavedBudget) => {
    navigate('/orcamento/obra', { state: { project: budget.project, savedBudgetId: budget.id } });
  };
